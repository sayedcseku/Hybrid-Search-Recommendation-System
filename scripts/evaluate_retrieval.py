...
def build_tf_datasets(train_df: pd.DataFrame, business_df: pd.DataFrame, batch_size: int, use_image: bool, img_df=None):
    ...
    # candidate ds for adapting text
    cand = {
...
def main():
    ...
    ap.add_argument("--image_embeddings", default=None, help="CSV/Parquet with per-business image_emb_0..")
    ap.add_argument("--k", type=int, default=None, help="override K for evaluation (defaults to config.retrieval.top_k)")
    args = ap.parse_args()
    ...
    # tf ds
    train_ds, cand_ds = build_tf_datasets(
    ...
    model.adapt_text(train_ds, cand_ds)

    model.compile(optimizer=tf.keras.optimizers.Adagrad(learning_rate=float(cfg["training"]["learning_rate"])))
    model.fit(train_ds, epochs=int(cfg["training"]["epochs_retrieval"]), verbose=1)
    ...
    engine = HybridEngine(
        ...
        top_k_retrieval=int(args.k or cfg["retrieval"]["top_k"]),
        ranking_model=None,
    )
    ...
    # evaluate: for each test instance, retrieve top-K and check if held-out item is in list
    for row in tqdm(test_df.itertuples(index=False), total=len(test_df)):
        retrieved = engine.retrieve(user_id=row.user_id, query_text="")  # user-only eval
        recalls.append(recall_at_k(retrieved, row.business_id, k_eval))
        ndcgs.append(ndcg_at_k(retrieved, row.business_id, k_eval))
